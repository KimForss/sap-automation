...
function load_config_vars() {
	...
	for var_name; do # iterate over function params
		...
		var_value="$(grep -m1 "^${var_name}=" "${var_file}" | cut -d'=' -f2- | tr -d '\r"' | xargs || true)"

		if [ -z "${var_value}" ]; then
			var_value="$(grep -m1 "^${var_name}[[:space:]]=" "${var_file}" | cut -d'=' -f2- | tr -d '\r"' | xargs || true)"
		fi
		...
		*)
			if is_valid_guid "${az_exec_user_name}"; then
				...
				if [ -z "$az_client_secret" ]; then
					...
					stty -echo
					read -ers -p "        -> Kindly provide SPN Password: " az_client_secret
					echo "********"
					stty echo
				fi
