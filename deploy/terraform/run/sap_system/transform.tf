...
locals {
  ...
  key_vault                            = {
                                           ...
                                           spn                                    = {
                                                                                      id     = coalesce(data.terraform_remote_state.deployer.outputs.deployer_kv_user_arm_id, var.spn_keyvault_id)
                                                                                      exists = length(coalesce(data.terraform_remote_state.deployer.outputs.deployer_kv_user_arm_id, var.spn_keyvault_id)) > 0
                                                                                    }
